...
import { deleteLanguage, moveLanguage } from '@/lib/actions/language'
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-100">어학</h1>
        <Button asChild>
          ...
          {items.map((item, idx) => (
            ...
            >
              <div className="flex flex-col gap-1">
                <form action={moveLanguage.bind(null, item.id, 'up')}>
                  <button
                    type="submit"
                    disabled={idx === 0}
                ...
                <form action={moveLanguage.bind(null, item.id, 'down')}>
                  <button
                    type="submit"
              ...
              <div className="flex-1">
                <div className="font-medium text-slate-200">
                  {item.name}
                  <span className="text-slate-400"> · {item.proficiency}</span>
                </div>
                {(item.testName || item.score) && (
                  <div className="text-sm text-slate-500">
                    {item.testName} {item.score && ` · ${item.score}`}
                  </div>
                )}
              ...
              <div className="flex gap-2">
                ...
                <DeleteButton action={deleteLanguage.bind(null, item.id)} />
              </div>
            </div>
          ))}
        </div>
