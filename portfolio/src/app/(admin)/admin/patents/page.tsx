...
export default async function PatentAdminPage() {
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-100">특허</h1>
        <Button asChild>
          ...
          {items.map((item, idx) => (
            ...
            >
              <div className="flex flex-col gap-1">
                <form action={movePatent.bind(null, item.id, 'up')}>
                  <button
                    type="submit"
                    disabled={idx === 0}
                ...
                <form action={movePatent.bind(null, item.id, 'down')}>
                  <button
                    type="submit"
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" asChild>
                  <Link href={`/admin/patents/${item.id}`}>
                    <Pencil size={14} />
                  </Link>
                </Button>
