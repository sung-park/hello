...
type SkillCategoryWithSkills = Prisma.SkillCategoryGetPayload<{
  include: { skills: true }
}>
...
export function ResumeContent({
  ...
  return (
    <article
      className={`doc-paper doc-resume mx-auto max-w-[210mm] bg-white p-12 shadow-sm print:p-0 print:shadow-none${className ? ` ${className}` : ''}`}
    >
      {/* Header */}
                ...
                <li key={exp.id} className="doc-entry doc-exp-entry">
                  <div className="doc-exp-header flex items-baseline justify-between gap-3">
                    <div>
                      ...
                      {expLocation && <span className="text-slate-500"> · {expLocation}</span>}
                    </div>
                  ...
                  {expAchievements && (
                    <div className="prose prose-sm max-w-none mt-1.5 text-sm text-slate-700 [&_li]:my-0.5 [&_p]:my-1 [&_ul]:my-1">
                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{expAchievements}</ReactMarkdown>
                    </div>
                  )}
                </li>
              )
        ...
        <section className="doc-section mb-5">
          ...
          {(() => {
            ...
            const granted = patents
              ...
              .reduce((acc, p) => acc + (p.count || 1), 0)
            const filed = patents
              .filter((p) => p.status === 'filed')
              .reduce((acc, p) => acc + (p.count || 1), 0)
            const mixed = patents
              .filter((p) => p.status === 'mixed')
              .reduce((acc, p) => acc + (p.count || 1), 0)
            const parts = [
